...
async def get_users(client: Bot, message: Message):
    ...
    # users = await full_userbase() # Eror mulu pake ini wkwk
    users = await query_msg()
    pengguna = []
    for row in users:
        pengguna.append(int(row[0]))
    try:
        await msg.edit(f"{len(pengguna)} <b>Pengguna menggunakan bot ini</b>")
    except:
        pass
